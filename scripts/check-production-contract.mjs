...
if(!/CREATE TABLE tenant_consumption_receipts/.test(consumptionReceiptMigration)||!/snapshot_sha256 char\(64\)/.test(consumptionReceiptMigration)||!/metric_ranges jsonb/.test(consumptionReceiptMigration))failures.push("Consumption proof must persist exact aggregate dashboard receipts");
if(!/security_barrier=true/.test(consumptionReceiptMigration)||!/tenant_scoped_consumption_receipts/.test(consumptionReceiptMigration)||!/pgi_require_tenant_context/.test(consumptionReceiptMigration))failures.push("Consumption receipts must remain tenant-scoped");
if(!/BEFORE UPDATE OR DELETE ON tenant_consumption_receipts/.test(consumptionReceiptMigration)||!/receipts are immutable/.test(consumptionReceiptMigration))failures.push("Consumption receipts must be immutable");
if(!/createCustomerConsumptionReceipt/.test(postgresStore)||!/reconcileTenantConsumptionReceipt/.test(postgresStore)||!/row\.metric_ranges/.test(postgresStore)||!/authoritative_call_facts_and_validated_tenant_distributions/.test(postgresStore))failures.push("Consumption proof must recompute the exact stored dashboard semantics from authoritative sources");
if(!/\/api\/v1\/customer\/consumption-receipts/.test(backendServer)||!/\/api\/v1\/platform\/tenants\/:id\/consumption-receipts\/:receipt\/reconcile/.test(backendServer)||!/\/api\/v1\/platform\/tenants\/:id\/consumption-today/.test(backendServer))failures.push("Consumption proof customer and staff endpoints are missing");
if(!/VOTRE NUMÉRO AUDIOTEL VALIDÉ/.test(clientAccountProofUi)||!/kyc_status/.test(clientAccountProofUi)||!/Copier le numéro/.test(clientAccountProofUi))failures.push("Customer portal must keep the validated assigned number prominent");
if(!/AUJOURD’HUI CÔTÉ SERVEUR/.test(tenantConsumptionCheckUi)||!/\/consumption-today/.test(tenantConsumptionCheckUi)||!/\/consumption-receipts/.test(tenantConsumptionCheckUi)||!/\/reconcile/.test(tenantConsumptionCheckUi)||!/CONFORME/.test(tenantConsumptionCheckUi)||!/ÉCART DÉTECTÉ/.test(tenantConsumptionCheckUi)||!/Copier le récapitulatif support/.test(tenantConsumptionCheckUi))failures.push("Customer 360 must expose current-day and mirror consumption reconciliation");
if(!/PGI Telecom/.test(clientPortalHtml)||!/Audiotel Premium Pro/.test(clientPortalHtml)||/PGI • Telecom - Audiotel Premium Pro/.test(clientPortalHtml))failures.push("Customer branding must show PGI Telecom with Audiotel Premium Pro without the internal cockpit name");
...
if(!/CREATE TABLE subscription_recovery_states/.test(subscriptionRecoveryMigration)||!/tenant_scoped_subscription_recovery/.test(subscriptionRecoveryMigration)||!/pgi_tenant_has_premium_call_access_v2/.test(subscriptionRecoveryMigration)||!/tenant_subscription_access_v2/.test(subscriptionRecoveryMigration))failures.push("subscription recovery migration must retain additive recovery state and v2 access objects");
if(!/interval \'7 days\'/.test(postgresStore)||!/interval \'14 days\'/.test(postgresStore))failures.push("subscription recovery runtime must retain 7-day grace and 14-day recovery windows");
