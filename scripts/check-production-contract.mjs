...
if(!/\/api\/v1\/customer\/consumption-receipts/.test(backendServer)||!/\/api\/v1\/platform\/tenants\/:id\/consumption-receipts\/:receipt\/reconcile/.test(backendServer))failures.push("Consumption proof customer and staff endpoints are missing");
...
if(!/CONFORME/.test(tenantConsumptionCheckUi)||!/ÉCART DÉTECTÉ/.test(tenantConsumptionCheckUi)||!/Copier le récapitulatif support/.test(tenantConsumptionCheckUi))failures.push("Customer 360 must expose mirror consumption reconciliation");
