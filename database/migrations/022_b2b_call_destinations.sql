...
CREATE TABLE IF NOT EXISTS tenant_call_destinations (
...
CREATE INDEX IF NOT EXISTS tenant_call_destinations_route_idx ON tenant_call_destinations(tenant_id,sva_number_id,status,priority,active_calls,last_assigned_at,id);
CREATE INDEX IF NOT EXISTS tenant_call_destinations_tenant_idx ON tenant_call_destinations(tenant_id,status,id);
ALTER TABLE calls ADD COLUMN IF NOT EXISTS call_destination_id bigint REFERENCES tenant_call_destinations(id);
ALTER TABLE calls ADD COLUMN IF NOT EXISTS call_destination_label text;
CREATE INDEX IF NOT EXISTS calls_destination_started_idx ON calls(call_destination_id,started_at DESC) WHERE call_destination_id IS NOT NULL;
