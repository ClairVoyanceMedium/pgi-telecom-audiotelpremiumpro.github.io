...
CREATE TABLE IF NOT EXISTS voice_sip_code_hourly_sharded (
  bucket_start timestamptz NOT NULL,
  market_id bigint NOT NULL REFERENCES operating_markets(id),
...
CREATE INDEX IF NOT EXISTS voice_sip_code_market_time_idx
  ON voice_sip_code_hourly_sharded(market_id,bucket_start DESC,sip_final_code);
CREATE INDEX IF NOT EXISTS voice_sip_code_carrier_time_idx
  ON voice_sip_code_hourly_sharded(host_carrier_id,bucket_start DESC,sip_final_code);

INSERT INTO voice_sip_code_hourly_sharded(
  bucket_start,market_id,host_carrier_id,sip_final_code,rollup_shard,calls_total,updated_at
)
SELECT
  date_trunc('hour',started_at),market_id,host_carrier_id,sip_final_code,(tenant_bucket%64)::smallint,
  count(*)::bigint,now()
FROM calls
WHERE market_id IS NOT NULL AND host_carrier_id IS NOT NULL AND sip_final_code BETWEEN 100 AND 699
GROUP BY date_trunc('hour',started_at),market_id,host_carrier_id,sip_final_code,(tenant_bucket%64)::smallint
ON CONFLICT(bucket_start,market_id,host_carrier_id,sip_final_code,rollup_shard) DO UPDATE SET
  calls_total=EXCLUDED.calls_total,updated_at=now();
