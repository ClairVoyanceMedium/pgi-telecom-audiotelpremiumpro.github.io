...
const clientBillingRecovery=read("assets/client-billing-recovery.js");
...
test("la Plateforme SVA distingue abonnement externe et usage interne exempté",()=>{
  ...
  assert.match(clientPortal,/assets\/client-billing-recovery\.js/);
  assert.match(clientPortalJs,/PGIBillingRecovery/);
  assert.match(clientBillingRecovery,/service_suspended/);
  assert.match(clientBillingRecovery,/next_retry_at/);
  assert.match(clientBillingRecovery,/createBillingPortal/);
