...
L’adaptateur Stripe Billing est isolé du cœur télécom. Les événements `invoice.paid`, `invoice.payment_failed` et `invoice.payment_action_required` relisent l’abonnement Stripe côté serveur avant normalisation afin de mettre à jour la période payée et le résultat du paiement sans faire confiance à des données de facture partielles.  Le webhook public `/api/v1/billing/stripe/webhook` vérifie d’abord cryptographiquement `Stripe-Signature` sur le corps brut, avec fenêtre anti-rejeu, puis normalise uniquement les événements d’abonnement utiles avant de les remettre au même moteur interne append-only. La route privée `/internal/billing/subscription-event` reste disponible pour un adaptateur séparé, mais n’est jamais utilisée comme webhook Stripe public.
