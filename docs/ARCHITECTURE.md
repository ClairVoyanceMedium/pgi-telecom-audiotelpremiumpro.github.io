...
L’adaptateur Stripe Billing est isolé du cœur télécom. Les événements `invoice.paid`, `invoice.payment_failed` et `invoice.payment_action_required` relisent l’abonnement Stripe côté serveur avant normalisation afin de ne jamais faire confiance à un simple retour navigateur ou à une facture partielle. Le webhook public `/api/v1/billing/stripe/webhook` vérifie cryptographiquement `Stripe-Signature` sur le corps brut, avec fenêtre anti-rejeu, puis alimente le journal append-only.

Le recouvrement est un état séparé de l’abonnement et du flux SVA. Un premier échec ouvre une grâce de 7 jours ; Stripe peut poursuivre ses relances pendant une fenêtre PGI de 14 jours. Pendant la grâce, le routage déjà actif reste disponible. Après la grâce, l’accès SVA est suspendu, mais le compte, les preuves, factures et reversements SVA acquis ne sont ni supprimés ni confisqués. Un paiement réussi réactive automatiquement l’accès. La fonction d’accès v2 et la vue v2 sont ajoutées sans remplacer les objets historiques afin de respecter la politique de migrations expand-only.

Le parcours client utilise Stripe Checkout hébergé : e-mail, tenant, version tarifaire et chemins de retour sont calculés côté serveur. Le prix Stripe est résolu par une `lookup_key` stable puis vérifié contre le catalogue PGI (montant, devise, périodicité et comportement fiscal) avant la création de Checkout. PGI Telecom • Audiotel Premium Pro ne stocke aucun numéro de carte. Le retour navigateur après paiement n’accorde jamais l’accès SVA à lui seul : seul un événement Stripe signé, validé et lié au bon tenant peut faire évoluer l’état d’abonnement.

Les routes `/customer/billing/status`, `/customer/billing/checkout-session` et `/customer/billing/portal-session` sont protégées par la session client et le CSRF. Checkout impose une clé d’idempotence propagée jusqu’à Stripe. Le Customer Portal Stripe permet la mise à jour du moyen de paiement, l’historique de facturation et la résiliation en fin de période. Tant que les secrets Stripe ne sont pas présents, le système reste fail-closed et répond `PAYMENT_PROVIDER_NOT_CONNECTED`.

Le branchement Stripe est activable uniquement par configuration d’environnement. Le flux abonnement reste strictement distinct du flux SVA : Stripe encaisse l’abonnement plateforme, tandis que les reversements SVA suivent toujours l’opérateur vers PGI puis le client après rapprochement et contrôles.
