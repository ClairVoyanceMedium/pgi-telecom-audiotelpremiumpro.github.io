...
export class MemoryStore{
  ...
  async selfServiceRegister(input={},passwordHash){
    ...
    if(String(input.legal_version||"")!=="2026-09-26")throw problem(409,"LEGAL_DOCUMENT_VERSION_OUTDATED");
    this.customerLegalAcceptances.push({acceptance_type:"account_terms",document_version:"2026-09-26",accepted_at:new Date().toISOString()});
  ...
  async recordCustomerLegalAcceptance(tenantId,principalId,input={}){
    if(String(input.document_version||"")!=="2026-09-26")throw problem(409,"LEGAL_DOCUMENT_VERSION_OUTDATED");
    const row={public_id:randomUUID(),tenant_id:Number(tenantId),customer_principal_id:String(principalId),acceptance_type:String(input.acceptance_type||""),document_version:"2026-09-26",immediate_performance_requested:input.immediate_performance_requested===true,accepted_at:new Date().toISOString()};
