...
export class PostgresStore{
  ...
  async customerBillingPreparation(tenantId){
    ...
    return this.withTenantReadContext(id,async tx=>{
      const tenant=(await tx.unsafe(
        "SELECT t.id,t.public_id,t.display_name,t.legal_name,t.tenant_type,t.status,t.country_code,t.billing_email,t.preferred_locale,t.default_currency,t.timezone,"+
        " CASE WHEN k.entity_type='individual' THEN 'individual' ELSE 'business' END AS customer_type FROM tenants t LEFT JOIN tenant_kyc_profiles k ON k.tenant_id=t.id WHERE t.id=$1 LIMIT 1",
        [id]
      ))[0];
      if(!tenant)throw problem(404,"TENANT_NOT_FOUND");
      if(tenant.tenant_type==="internal")throw problem(409,"INTERNAL_TENANT_BILLING_EXEMPT");
