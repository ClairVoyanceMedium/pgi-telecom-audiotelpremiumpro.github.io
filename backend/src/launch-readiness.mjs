...
export function evaluateLaunchReadiness(input={}){
  const config=input.config||{},system=input.system||{},performance=input.performance||{},platform=input.platform||{},billing=input.billingProvider||{},carrier=input.carrier||{},withdrawalReady=input.withdrawalReady===true;
  ...
  const billingReady=billing.connection_state==="connected"&&billing.stripe_live_mode===true;
  ...
  const sections=[
    ...
    section("billing","Paiements Stripe",billingReady?STATUS.READY:STATUS.ACTION,billingReady?"Stripe est connecté en mode live.":"Stripe live doit être connecté et opérationnel avant encaissement.",["b2b","b2c"]),
