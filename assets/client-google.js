...
async function activate(el,options,clientId){
  await load();
...
async function init(options){
  ...
  [options.loginElement,options.activationElement].forEach(function(el){
    if(!el)return;
    el.hidden=false;el.innerHTML='<button type="button" class="cp-ghost" data-google-enable>Continuer avec Google</button>';
    el.querySelector("[data-google-enable]").addEventListener("click",function(){activate(el,options,clientId).catch(function(){});},{once:true});
  });
  document.querySelectorAll("[data-google-note]").forEach(function(el){el.hidden=false;el.textContent="Le service Google n’est chargé qu’après votre action sur le bouton Google.";});
  return true;
}
