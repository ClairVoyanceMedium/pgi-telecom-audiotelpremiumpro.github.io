...
export async function render(slot,request){
  ...
  try{
    ...
    slot.innerHTML='<h3>Performance & Resilience Lab</h3><p class="ct-note">Preuves mesurées avant branchement opérateur. Aucun chiffre de capacité n’est présenté comme garanti tant qu’un test réel n’a pas été enregistré.</p>'+
      '<div class="ct-status">'+badge(gate.ready?"ready":"attention")+badge(data.capacity_proof)+'<span class="ct-badge">PROUVÉ '+(data.load?.latest_passed?nf(data.load.latest_passed.requests_per_second)+' req/s':'—')+'</span><span class="ct-badge">GLOBAL '+esc(limits.global_per_minute||0)+'/min</span><span class="ct-badge">HEAVY '+esc(limits.heavy_read_per_minute||0)+'/min</span><span class="ct-badge">WRITE '+esc(limits.write_per_minute||0)+'/min</span></div>'+
