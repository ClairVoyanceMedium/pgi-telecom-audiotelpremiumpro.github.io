...
function stamp(v){if(!v)return "—";try{return new Intl.DateTimeFormat(navigator.language||"fr-FR",{dateStyle:"short",timeStyle:"medium"}).format(new Date(v))}catch(_e){return String(v)}}
function ui(){
...
function values(){
 var d=s.data||{},cur=d.default_currency||"EUR",rows=d.by_currency||[];
 var row=rows.find(function(x){return x.currency===cur})||rows[0]||{};
 var elapsed=Math.max(0,Math.min(30,(Date.now()-(s.at||Date.now()))/1000)),rate=n(row.client_rate_ht_per_second);
...
async function reset(){
 ...
 try{await api.resetJackpot(api.newIdempotencyKey());s.data={...(s.data||{}),reset_at:new Date().toISOString(),as_of:new Date().toISOString(),by_currency:[]};s.at=Date.now();paint();if(m)m.textContent="Jackpot remis à 0,00 €. Le compteur repart dès cette seconde.";await refresh()}
