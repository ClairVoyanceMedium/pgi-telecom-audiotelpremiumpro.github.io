...
const automationLabels={queued:"Automatique · en file",checking:"Automatique · contrôle",submitting:"Automatique · envoi opérateur",operator_pending:"Automatique · suivi opérateur",scheduled:"Automatique · bascule planifiée",completing:"Automatique · activation",completed:"Automatique · terminé",action_required:"Exception · intervention Audiotel Premium Pro",failed:"Automatique · nouvelle tentative",cancelling:"Automatique · annulation",cancelled:"Automatique · annulé"};
...
export function renderPortabilitySection(rows=[],carrierAdmin={}){
  ...
  const cards=rows.map(p=>{
    ...
    return '<div class="td-port-card" data-portability-card="'+esc(p.id)+'" data-active-carrier-id="'+esc(activeCarrierId)+'"><div class="td-port-head"><div><strong>'+esc(p.display_number||p.requested_e164)+'</strong><small>'+esc(detail||p.country_code||"")+'</small></div>'+chip(p.status)+'</div>'+
      ...
      '<p class="td-port-note">Le numéro E.164 reste '+esc(p.requested_e164)+'. Audiotel Premium Pro traite normalement ce dossier automatiquement : contrôle, envoi opérateur, suivi et activation. Les commandes ci-dessous servent uniquement de reprise manuelle ou de traitement d’une exception.'+(p.automation_last_error?' Exception actuelle : '+esc(p.automation_last_error):'')+'</p>'+
...
export async function runPortabilityAction(e,api){
  ...
  if(complete){
    ...
    if(!confirm("Confirmer uniquement si l’opérateur a effectivement réalisé la bascule du numéro. Le même numéro et le tarif vérifié seront activés dans Audiotel Premium Pro."))return {handled:true,cancelled:true};
