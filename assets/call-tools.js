...
function metricLabel(el){
  const p=el&&el.parentElement,candidate=p&&p.querySelector("span:not([id]),small:not([id]),h2,h3");
  return candidate&&String(candidate.textContent||"").trim()||String(el.id||"Métrique").replace(/[-_]+/g," ");
}
function adminMetricRows(){
  const rows=[["Métrique","Valeur","Identifiant"]],seen=new Set();
  document.querySelectorAll(".main strong[id],.main span[id]").forEach(el=>{
    const id=String(el.id||"").trim(),value=String(el.textContent||"").replace(/\s+/g," ").trim();
    if(!id||!value||value.length>220||seen.has(id))return;
    seen.add(id);rows.push([metricLabel(el),value,id]);
  });
  return rows;
}
function adminReportRows(){
  return [["Cockpit / PGI Telecom • Audiotel Premium Pro"],["Export",new Date().toISOString()],["Période",readValue("command-period")],["Dernière synchro",readValue("command-sync")],[],["MÉTRIQUES DU COCKPIT"]].concat(adminMetricRows());
}
function rowsToPlainText(rows){return rows.map(r=>r.map(v=>String(v==null?"":v)).join("\t")).join("\n");}
...
function exportSummary(){
  downloadRows("pgi-audiotel-rapport-complet-"+new Date().toISOString().slice(0,10)+".csv",adminReportRows());
}
async function copySummary(){
  await copyPlainText(rowsToPlainText(adminReportRows()));
}
