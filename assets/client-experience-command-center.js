...
function renderAlerts(x){
  ...
  return '<article class="ccx-card"><div class="ccx-head"><div><div class="ccx-kicker">MES ALERTES</div><h3>Seuils personnels</h3></div><span class="ccx-status '+(count?"warn":"ok")+'">'+count+' active'+(count>1?"s":"")+'</span></div><div class="ccx-alerts">'+(alerts.length?alerts.map(v=>'<div class="ccx-alert">'+esc(v)+'</div>').join(""):'<div class="ccx-alert ok">Aucune alerte personnalisée déclenchée avec les données disponibles.</div>')+'</div><div class="ccx-actions"><button id="ccx-alert-settings" class="cp-ghost ccx-btn" type="button">Configurer mes seuils</button></div><p class="ccx-note">Pour l’instant, les alertes apparaissent dans le portail. Aucun SMS, e-mail ou push externe n’est branché.</p></article>';
}
