...
export async function runServiceOperation(e,api,tenantId){
  ...
  if(detail){
    ...
    const timeline=[
      ...(data.events||[]).map(x=>({at:x.occurred_at,who:x.actor_type==="customer"?"Client":x.actor_type==="staff"?"PGI":"Système",text:x.message||x.event_type})),
      ...(data.notes||[]).map(x=>({at:x.created_at,who:x.author_type==="customer"?"Client":x.author_type==="staff"?"PGI":"Système",text:x.body}))
