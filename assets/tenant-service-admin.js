...
export function renderServiceOperations(data){
  ...
  const rows=incidents.map(x=>'<div class="td-alert" data-service-incident="'+esc(x.public_id)+'"><div class="td-section-head"><strong>'+esc(x.title)+'</strong>'+serviceChip(x.status)+'</div><p>'+esc(lab(cats,x.category)+" · Priorité "+lab(priorities,x.severity)+" · équipe "+(x.assigned_team==="PGI Operations"?"Équipe Audiotel Premium Pro":(x.assigned_team||"Équipe Audiotel Premium Pro")))+'</p><small>Réponse cible '+esc(date(x.first_response_due_at))+' · résolution '+esc(date(x.target_resolution_at))+'</small><div class="td-actions"><button class="td-btn mini" data-service-detail>Historique</button><button class="td-btn mini" data-service-status="investigating">Prendre en charge</button><button class="td-btn mini" data-service-status="waiting_customer">Attendre le client</button><button class="td-btn mini" data-service-status="monitoring">Surveiller</button><button class="td-btn mini success" data-service-status="resolved">Résoudre</button></div><div data-service-detail-panel></div><div class="td-route-form"><input data-service-note-body maxlength="5000" placeholder="Message à ajouter au dossier"><button class="td-btn mini" data-service-note>Envoyer au client</button></div></div>').join("");
...
export async function runServiceOperation(e,api,tenantId){
  ...
  const detail=e.target.closest("[data-service-detail]");
  if(detail){
    const card=detail.closest("[data-service-incident]"),panel=card?.querySelector("[data-service-detail-panel]");
    const data=await api.serviceIncident(card.dataset.serviceIncident);
    const timeline=[
      ...(data.events||[]).map(x=>({at:x.occurred_at,who:x.actor_type==="customer"?"Client":x.actor_type==="staff"?"Audiotel Premium Pro":"Système",text:x.message||x.event_type})),
      ...(data.notes||[]).map(x=>({at:x.created_at,who:x.author_type==="customer"?"Client":x.author_type==="staff"?"Audiotel Premium Pro":"Système",text:x.body}))
    ].sort((a,b)=>Date.parse(a.at)-Date.parse(b.at));
    if(panel)panel.innerHTML='<div class="td-timeline">'+(timeline.length?timeline.map(x=>'<div class="td-event"><strong>'+esc(x.who)+'</strong><small>'+esc(date(x.at))+'</small><p>'+esc(x.text)+'</p></div>').join(""):'<p class="td-empty">Aucun événement.</p>')+'</div>';
    return {handled:true,message:"Historique chargé."};
  }
  ...
  const note=e.target.closest("[data-service-note]");
  if(note){
    const card=note.closest("[data-service-incident]"),body=card?.querySelector("[data-service-note-body]")?.value.trim()||"";
    if(!body)return {handled:true,error:"Message requis."};
    await api.addServiceIncidentNote(card.dataset.serviceIncident,{body,customer_visible:true},api.newIdempotencyKey());
    return {handled:true,message:"Message ajouté au dossier client."};
  }
