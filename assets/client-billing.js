export function createController(ctx){
  ...
  function ensureCancelDialog(){
    ...
    d.innerHTML='<div class="cp-dialog-card"><div class="cp-dialog-head"><div><p class="cp-kicker">RÉSILIATION ÉLECTRONIQUE</p><h2>Résilier mon abonnement</h2></div><button class="cp-dialog-close" type="button" data-cancel-close aria-label="Fermer">×</button></div><p class="cp-muted">Vérifiez les informations ci-dessous avant de notifier définitivement la résiliation.</p><div class="cp-stack"><div class="cp-row"><div><strong>Titulaire / compte</strong><span id="client-cancel-holder"></span></div></div><div class="cp-row"><div><strong>Contact électronique</strong><span id="client-cancel-contact"></span></div></div><div class="cp-row"><div><strong>Contrat</strong><span id="client-cancel-contract"></span></div></div><div class="cp-row"><div><strong>Date d’effet prévue</strong><span id="client-cancel-effective"></span></div></div><div class="cp-row"><div><strong>Numéros SVA</strong><span>Cette résiliation concerne l’abonnement plateforme. Elle ne demande ni portabilité ni libération automatique d’un numéro.</span></div></div></div><p class="cp-muted">Après notification, la demande reçoit une référence horodatée. Sauf droit impératif ou situation particulière, l’abonnement reste accessible jusqu’à la fin de la période déjà payée puis cesse de se renouveler.</p><div class="cp-search-scopes"><button id="client-cancel-confirm" class="cp-primary" type="button">Notifier la résiliation</button><button type="button" data-cancel-close>Retour</button></div></div>';
  ...
  function openCancellation(){
    ...
    $("client-cancel-contact").textContent=data.billing_summary?.checkout_prefill?.email||data.user?.email||"Contact du compte authentifié";
