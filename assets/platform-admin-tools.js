...
const CONTROL_LABEL=Object.fromEntries(ARCEP_2026_CONTROLS);
...
function renderComplianceEditor(assignmentId){
  const editor=$("pa-compliance-editor"),numbers=lastPlatform?.regulatory_trust?.numbers||[];
  const row=numbers.find(x=>String(x.assignment_id)===String(assignmentId));
  if(!editor||!row)return;
  const checks=ARCEP_2026_CONTROLS.map(([key,label])=>{
    const status=row[ARCEP_FIELD_BY_CONTROL[key]]||"not_started";
...
async function saveArcepEvidence(){
  ...
  if(!assignmentId||!CONTROL_LABEL[control])return feedback("Contrôle ARCEP invalide.","error");
  if(status==="verified"&&!reference)return feedback("Une référence de preuve est obligatoire pour marquer ce contrôle comme vérifié.","error");
  const label=CONTROL_LABEL[control]||control;
  if(!confirm("Ajouter un événement de preuve « "+status+" » pour : "+label+" ?"))return;
  busy=true;feedback("Enregistrement de la preuve ARCEP 2026…");
  try{
    const data=await postJson("/platform/tenant-number-assignments/"+encodeURIComponent(assignmentId)+"/regulatory-evidence",{control_key:control,status:status,source:source,evidence_reference:reference||null,metadata:{recorded_from:"cockpit_arcep_2026"}});
    busy=false;feedback("Preuve ajoutée à la chaîne immuable. État ARCEP 2026 recalculé.","ok");
    await load();
    renderComplianceEditor(assignmentId);
    window.dispatchEvent(new CustomEvent("pgi:command",{detail:{id:"refresh"}}));
    return data;
  }catch(err){busy=false;feedback(err.code||"Enregistrement impossible","error");}
}
async function handle(e){
  if(busy)return;
  const open=e.target.closest("[data-regulatory-open]");
  if(open){renderComplianceEditor(open.dataset.regulatoryOpen);return;}
  if(e.target.closest("[data-arcep-evidence-save]")){await saveArcepEvidence();return;}
  ...
  if(e.target.closest("[data-switch-plan]")){
    ...
    try{await window.PGIApi.planCarrierSwitch({route_key:"sva-primary",to_carrier_id:carrierId,connection_id:connectionId,rollback_window_minutes:rollback,notes:"Préparée depuis le cockpit"},window.PGIApi.newIdempotencyKey());busy=false;feedback("Bascule préparée. Vérifier puis utiliser Activer.","ok");await load();}catch(err){busy=false;feedback(err.code||"Préparation impossible","error");}return;
  }
  const activate=e.target.closest("[data-switch-activate]");if(activate){if(!confirm("Activer maintenant cette bascule opérateur ?"))return;busy=true;feedback("Activation atomique…");try{await window.PGIApi.activateCarrierSwitch(activate.dataset.switchActivate,window.PGIApi.newIdempotencyKey());busy=false;feedback("Nouvelle route activée.","ok");await load();window.dispatchEvent(new CustomEvent("pgi:command",{detail:{id:"refresh"}}));}catch(err){busy=false;feedback(err.code||"Activation impossible","error");}return;}
