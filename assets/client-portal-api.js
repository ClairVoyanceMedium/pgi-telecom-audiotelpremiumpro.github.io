...
register:function(payload){return request("/customer/auth/register",{method:"POST",body:Object.assign(payload,root.PGIOrderMeta||{})});},
...
forgotPassword:function(email){return request("/customer/auth/password/forgot",{method:"POST",body:{email:email}});},
resetPassword:function(token,newPassword){return request("/customer/auth/password/reset",{method:"POST",body:{token:token,new_password:newPassword}});},
requestEmailChange:function(newEmail,currentPassword){return request("/customer/auth/email/change/request",{method:"POST",body:{new_email:newEmail,current_password:currentPassword}});},
confirmEmailChange:function(token){return request("/customer/auth/email/change/confirm",{method:"POST",body:{token:token}});},
passkeys:function(){return request("/customer/security/passkeys",{timeoutMs:5000});},
passkeyRegisterOptions:function(){return request("/customer/security/passkeys/register-options",{method:"POST",body:{}});},
passkeyRegister:function(body){return request("/customer/security/passkeys/register",{method:"POST",body:body});},
passkeyAssertOptions:function(){return request("/customer/security/passkeys/assert-options",{method:"POST",body:{}});},
passkeyVerify:function(body){return request("/customer/security/passkeys/verify",{method:"POST",body:body});},
...
resetJackpot:function(k){return request("/customer/jackpot/reset",{method:"POST",body:{},idempotencyKey:k});},
...
createExitRequest:function(payload,idempotencyKey){return request("/customer/relations/exits",{method:"POST",body:payload,idempotencyKey:idempotencyKey});},
addRelationMessage:function(id,body,idempotencyKey){return request("/customer/relations/"+encodeURIComponent(id)+"/messages",{method:"POST",body:{body:body},idempotencyKey:idempotencyKey});},
cancelRelationCase:function(id,idempotencyKey){return request("/customer/relations/"+encodeURIComponent(id)+"/cancel",{method:"POST",body:{},idempotencyKey:idempotencyKey});},
confirmRelationAction:function(id,idempotencyKey){return request("/customer/relations/actions/"+encodeURIComponent(id)+"/confirm",{method:"POST",body:{},idempotencyKey:idempotencyKey});},
