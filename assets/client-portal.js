...
var billingController=null,billingPromise=null;
function ensureBilling(){
  if(billingController)return Promise.resolve(billingController);
  if(!billingPromise)billingPromise=import("./client-billing.js").then(function(m){billingController=m.createController({$: $,tr:tr,money:money,n:n,dateOnly:dateOnly,esc:esc,chip:chip,getState:function(){return state;},toast:toast});return billingController;});
  return billingPromise;
}
function renderSubscriptions(data){ensureBilling().then(function(x){x.render(data);}).catch(function(){toast("Facturation momentanément indisponible.");});}
