...
export async function mountTeamAccess(pane,api,user,flash){
  ...
  pane.innerHTML=
    ...
    (flash?.activation_path?'<div class="pp-item" data-team-flash><strong>Invitation créée</strong><p>Copiez ce lien et transmettez-le au collaborateur concerné. Aucun e-mail automatique n’a été envoyé.</p><code>'+esc(flash.activation_path)+'</code><button data-team-copy type="button">Copier le lien</button></div>':'')+
  ...
  pane.querySelector("[data-team-copy]")?.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(String(flash?.activation_path||""));const b=pane.querySelector("[data-team-copy]");if(b)b.textContent="Copié"}catch(_e){}});
