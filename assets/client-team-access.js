...
export async function mountTeamAccess(pane,api,user,flash){
  ...
  const members=state.members||[],invites=state.invitations||[],activationLink=activationLink?new URL(flash.activation_path,window.location.href).href:"";
  pane.innerHTML=
    ...
    (activationLink?'<div class="pp-item" data-team-flash><strong>Invitation créée</strong><p>Copiez ce lien et transmettez-le au collaborateur concerné. Aucun e-mail automatique n’a été envoyé.</p><code>'+esc(flash.activation_path)+'</code><button data-team-copy type="button">Copier le lien</button></div>':'')+
  ...
  pane.querySelector("[data-team-copy]")?.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(String(activationLink||""));const b=pane.querySelector("[data-team-copy]");if(b)b.textContent="Copié"}catch(_e){}});
